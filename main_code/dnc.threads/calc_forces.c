...
extern int nThread; /* delcared in dnc.c */
...
void calc_forces(Node *particles, int N, float eps)
{
	...
	calc_interaction_threaded(&rootNode);
...
void calc_interaction_threaded(Node *root){
  ...
  int cr, ct;
  void *retval;
  int nPerThread, nChildren;
  Node *nList[8];
  InteractionPair **ips;
  Node *n;
  pthread_t *threads;

  // load the nodes into an easier to handle list
  nChildren = 0;
  n = CHILD(root);
  while(n != NULL){
    nList[nChildren] = n;
  ...
  // allocate memory for the interaction lists
  nPerThread = 1.0+ceil(0.5*nChildren*(nChildren+1)/nThread);
  ips = (InteractionPair **)calloc(nThread, sizeof(InteractionPair *));
  for (i=0;i<nThread;i++){
    ips[i] = (InteractionPair *)calloc(nPerThread, sizeof(InteractionPair));
  }
  // allocate the threads
  threads = (pthread_t *)calloc(nThread, sizeof(pthread_t));


  // now, make the interaction list for each thread (each list is null terminated)
  // (note: array was calloc'd so unset values are NULL)
  ct = 0;
  cr = 0;
  for (i=0;i<nChildren;i++){
    //printf("%d,%d\t%d,%d\t%d, %d\n", ct, cr, i, j, nList[i], nList[j]);
    ips[ct][cr].n1 = nList[i];
    ips[ct][cr].n2 = nList[i];
    ips[ct][cr].i = i;
    ips[ct][cr].j = i;
    ct++;
    if (ct == nThread){
  ...
  // finally, launch each thread
  for (i=1;i<nThread;i++){
    //printf("Starting thread %d\n", i);
    pthread_create(threads+i, NULL, process_list, (void *)(ips[i]));
  }
  // and my own
  process_list(ips[0]);
  ...
  //free the memory from the lists
  free(threads);
  for (i=0;i<nThread;i++){
    free(ips[i]);
  }
...
//----------------------------------------------------------------------
// calculate the interactions between each node in the list
//----------------------------------------------------------------------
void *process_list(void *ptr){
  int i;
  //float tinterval;
  ...
  ip = (InteractionPair *)ptr;
  //tstart = clock();
  i = 0;
  while (ip[i].n1 != NULL){
    //printf("%d\n", i);
    calc_interaction(ip[i].n1, ip[i].n2);

    i++;
  }
  //tstop = clock();
  //tinterval = 1.0*(tstop - tstart)/CLOCKS_PER_SEC;

  pthread_mutex_lock(&outputlock);
  //printf("%.3f: ", tinterval);
  i = 0;
  while (ip[i].n1 != NULL){
    //printf("%d,%d\t", ip[i].i, ip[i].j);
    i++;
  }
  //printf("\n");
  fflush(stdout);
...
void calc_coefficients_pp(Node *nodeA, Node *nodeB, const float *R)
{
	pthread_mutex_lock(LOCK(nodeA));	
	VECTOR_MUL_ADD(D1(nodeA), MASS(nodeB), R);
	pthread_mutex_unlock(LOCK(nodeA));
	
    pthread_mutex_lock(LOCK(nodeB));
	VECTOR_MUL_ADD(D1(nodeB), -MASS(nodeA), R);
	pthread_mutex_unlock(LOCK(nodeB));
}
